...
local loc_src = NFS.getDirectoryItems(SMODS.find_mod("JoyousSpring")[1].path .. "localization/default")
for _, file in ipairs(loc_src) do
	if NFS.getInfo(SMODS.find_mod("JoyousSpring")[1].path .. "localization/default/" .. file).type ~= "directory" then
		localization_files[#localization_files + 1] = assert(SMODS.load_file("localization/default/" .. file,
			"JoyousSpring"))()
	end
end

local loc_archetypes_src = NFS.getDirectoryItems(SMODS.find_mod("JoyousSpring")[1].path ..
	"localization/default/archetypes")
for _, file in ipairs(loc_archetypes_src) do
	localization_files[#localization_files + 1] = assert(SMODS.load_file("localization/default/archetypes/" .. file,
		"JoyousSpring"))()
end
